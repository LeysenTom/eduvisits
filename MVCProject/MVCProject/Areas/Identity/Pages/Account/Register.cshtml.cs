...
namespace MVCProject.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task SetRolesToUser(Gebruiker user)
        {
            ...
            if (Input.SecretariaatsMedewerker)
            {
            ...
            if (Input.Beheerder)
            {
                var rol = rolLijst.Where(r => r.Name.Contains("Beheerder")).ToList().FirstOrDefault();
                if (user != null && rol != null)
                    await _userManager.AddToRoleAsync(user, rol.Name);
            }
        }
